"""
=============================================================
UNet Semantic Segmentation Training on NVIDIA A100 (CUDA)
=============================================================
...
def train_epoch(model, loader, criterion, optimizer, device, epoch, num_epochs):
    ...
    for batch_idx, (images, masks) in enumerate(pbar):
        ...
        iter_time = batch_time  # Same for single iteration
        
        mem_alloc = torch.cuda.memory_allocated() / 1024**3 if torch.cuda.is_available() else 0
        mem_res = torch.cuda.memory_reserved() / 1024**3 if torch.cuda.is_available() else 0
...
def main():
    print(f"\n{'='*60}")
    print("UNet Training - NVIDIA CUDA")
    print(f"{'='*60}\n")
    ...
    for epoch in range(CONFIG['num_epochs']):
        ...
        if val_metrics['mean_iou'] == best_val_iou:
            # Recursively move all tensors to CPU for saving
    ...
    print(f"{'='*60}\n")
    model.load_state_dict(torch.load('best_unet_cuda.pth')['model_state_dict'])
    test_metrics = validate(model, test_loader, criterion, device)
    
    # Final Results
    print(f"\n{'='*60}")
    print("FINAL RESULTS - NVIDIA CUDA")
    print(f"{'='*60}")
    print(f"\nTiming:")
    ...
    with open('results_cuda.json', 'w') as f:
        json.dump(results, f, indent=2)
    
    print(f"\n✓ Results saved to results_cuda.json")
    print(f"{'='*60}\n")
