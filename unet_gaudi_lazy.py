"""
=============================================================
UNet Semantic Segmentation Training on Intel Gaudi (Lazy Mode)
=============================================================
...
if HABANA_AVAILABLE and torch.hpu.is_available():
    ...
    print(f"\n{'='*60}")
    print(f"Device: HPU (Intel Gaudi - Lazy Mode)")
...
# =============================================================
# 5. Training & Validation (Modified for Gaudi Lazy Mode)
# =============================================================
...
def train_epoch(model, loader, criterion, optimizer, device, epoch, num_epochs):
    ...
    for batch_idx, (images, masks) in enumerate(pbar):
        ...
        # LAZY MODE: Mark step after optimizer
        if HABANA_AVAILABLE:
            htcore.mark_step()
...
def main():
    print(f"\n{'='*60}")
    print("UNet Training - Intel Gaudi (Lazy Mode)")
    print(f"{'='*60}\n")
    ...
    print(f"{'='*60}\n")
    model.load_state_dict(torch.load('best_unet_gaudi_lazy.pth')['model_state_dict'])
    test_metrics = validate(model, test_loader, criterion, device)
    
    # Final Results
    print(f"\n{'='*60}")
    print("FINAL RESULTS - Intel Gaudi (Lazy Mode)")
    print(f"{'='*60}")
    print(f"\nTiming:")
    ...
    with open('results_gaudi_lazy.json', 'w') as f:
        json.dump(results, f, indent=2)
    
    print(f"\n✓ Results saved to results_gaudi_lazy.json")
    print(f"{'='*60}\n")
